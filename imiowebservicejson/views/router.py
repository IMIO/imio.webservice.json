...
class PostRouterSubscriptionSchema(colander.MappingSchema):
    ...
    url = colander.SchemaNode(
        colander.String(), description="The url of the application"
    )


class RouterResponseSchema(colander.MappingSchema):
    msg = colander.SchemaNode(colander.String(), description="The response message")
...
response_schemas = {"200": RouterSuccessResponseSchema(description="Return value")}
...
def get_discovery_route(request):
    parameters = {"client_id": request.matchdict["client_id"]}
